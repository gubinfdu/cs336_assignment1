...
def merge_byte_counter(byte_counter, pair_counter, merge_pair):
    # pair_counter = pair_counter.copy()
    # pair_counter.pop(merge_pair)
    new_byte = merge_pair[0] + merge_pair[1]
    new_byte_counter = {}
    affect_list = []
    for k, v in byte_counter.items():
        merge_status = False
        # k: (bytes1, bytes2 ...), v: int
        # update byte_counter
        if (merge_pair[0] not in k) or (merge_pair[1] not in k):
            new_byte_counter[k] = v
            continue
        new_k = []
        l = len(k)
        i = 0
        while i < l:
            if (i < l - 1) and (k[i] == merge_pair[0]) and (k[i + 1] == merge_pair[1]):
                # merge byte_counter
                new_k.append(new_byte)
                i += 2
                merge_status = True
            else:
                new_k.append(k[i])
                i += 1
        new_byte_counter[tuple(new_k)] = v

        # update pair_counter
        for i in range(len(k) - 1):
            pair_counter[(k[i], k[i + 1])] -= v
        for i in range(len(new_k) - 1):
            temp_k = (new_k[i], new_k[i + 1])
            if temp_k not in pair_counter:
                pair_counter[temp_k] = 0
            pair_counter[temp_k] += v

    return new_byte_counter, pair_counter
...
def train_bpe(input_path: str, vocab_size: int, special_tokens: List[str], train_mode: bool = True) -> Any:
    ...
    for i in range(vocab_size - len(vocab)):
        ...
        byte_counter, pair_counter = merge_byte_counter(byte_counter, pair_counter, merge_pair)
        t3 = time.time()
