...
class RotaryPositionalEmbedding(nn.Module):
    ...
    def forward(self, x, token_positions=None):
        ...
        '''
        *batch_size, seq_len, d_k = x.shape
        inds = torch.flip(torch.arange(d_k).reshape(-1, 2), dims=[-1]).reshape(-1, ).to(torch.int).to(x.device)
        signs = torch.ones(d_k).reshape(-1, 2)
        signs[:, 0] *= -1
        signs = signs.reshape(-1, ).to(torch.int).to(x.device)
        x2 = torch.index_select(x, -1, inds) * signs

        if token_positions is None:
            token_positions = torch.arange(seq_len).to(torch.int).to(x.device)
        else:
